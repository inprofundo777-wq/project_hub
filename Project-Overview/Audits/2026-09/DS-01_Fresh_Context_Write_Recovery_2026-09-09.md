...
Этот раздел заполняется фактическим результатом Post-Write Check после создания файла. До обратного чтения PASS не объявляется.
...
**Verdict pending Post-Write Check.**

PASS допустим только если обратное чтение подтвердит точный путь, статус, полноту и содержание этого файла и не будет обнаружено незапланированных изменений.
